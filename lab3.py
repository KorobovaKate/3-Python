...
def encode_green(image_path, keys_path, message, output_path):
    ...
    valid_keys = [(x, y) for x, y in keys if x < img.width and y < img.height]

    if len(message) > len(valid_keys):
        print(f"Ошибка: Сообщение слишком длинное ({len(message)}), максимум {len(valid_keys)} символов.")
        return False
    ...
    for i, (x, y) in enumerate(valid_keys):
        ...
        if i < len(message):
            ...
            if char_code > 255:
                print(f"Символ '{message[i]}' вне диапазона ASCII. Заменён на '?'.")
                char_code = ord('?')
